...
void perform_ref_masking(PictureControlSet *pcs_ptr, ModeDecisionContext *context_ptr,
...
#if ADD_INTRA
    uint32_t early_intra_distortion = early_intra_evaluation(pcs_ptr,
        context_ptr,
        input_picture_ptr,
        input_origin_index,
        blk_origin_index,
        ...
        for (uint32_t ri = 0; ri < REF_LIST_MAX_DEPTH; ri++) {
            if (context_ptr->ref_filtering_res[li][ri].valid_ref) {
                if (context_ptr->ref_filtering_res[li][ri].dist > early_intra_distortion) {
                    if ((context_ptr->ref_filtering_res[li][ri].dist - early_intra_distortion) * 100 >
                        th * early_intra_distortion) {
    ...
    //if after intra-to-inter distortion check, less than 2 ref are tagged, then tag the best 2 ref 
